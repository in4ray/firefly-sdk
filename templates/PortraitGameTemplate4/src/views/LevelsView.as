package views
{
	...
	public class LevelsView extends Sprite
	{
		...
		private var dataRenderer:DataRenderer;
		...
		public function LevelsView()
		{
			...
			map.numScreens = Math.ceil(gameModel.currentBundle.value.levels.length/15);// Number of screens equals number of levels
			map.duration = 500;
			map.sensitivity = 20;
			...
			// Levels renderers
			dataRenderer = new DataRenderer(new LevelsTemplate(map));
			dataRenderer.itemRenderer = new ClassFactory(LevelItemRenderer);
			dataRenderer.dataProvider = gameModel.currentBundle.value.levels;
			...
			gameModel.currentBundle.bindListener(currentBundleChanged);
		}
		
		private function currentBundleChanged(e:BindingEvent):void
		{
			dataRenderer.dataProvider = gameModel.currentBundle.value.levels;
		}
		...
		{
			dispatchEvent(new ViewStateEvent(ViewStateEvent.SWITCH_TO_STATE, ViewStates.BUNDLES));
		}
